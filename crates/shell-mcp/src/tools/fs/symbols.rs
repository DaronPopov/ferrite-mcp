...
fn collect_rs_files(root: &Path) -> Vec<PathBuf> {
    // Drive the unified walker, restricted to .rs files. Gitignore is
    // honoured automatically, so target/, .git/, and node_modules/ are
    // pruned without an explicit skip list.
    ...
    iter.filter(|e| matches!(e.file_type, FileType::File))
        .map(|e| e.path)
        .collect()
}
