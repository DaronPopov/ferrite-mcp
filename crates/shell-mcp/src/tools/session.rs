//! session_status — report the state of the auto-started cream/claude session.
//!
//! Reads:
//!   - tmux session alive?
//!   - Saved remote-control URL (~/.local/share/cream/remote-session-url.txt)
//!   - autostart log tail (~/.local/share/cream/autostart.log)
//!   - systemd unit status
...
pub fn session_status(_args: &Value) -> Result<ToolResult, String> {
    ...
    // ── tmux session ──────────────────────────────────────────────────────────
    let session_name = std::env::var("CREAM_SESSION").unwrap_or_else(|_| "claude-remote".to_owned());
    let tmux_raw = run(
    ...
    // ── remote URL ────────────────────────────────────────────────────────────
    let url_file = home.join(".local/share/cream/remote-session-url.txt");
    ...
    let log_file = home.join(".local/share/cream/autostart.log");
    ...
    // ── systemd unit status ───────────────────────────────────────────────────
    let systemd_raw = run(
        "systemctl --user is-active cream-session.service 2>/dev/null || echo inactive",
        &cwd, &[], "", Duration::from_secs(5),
    );
    ...
    let ntfy_configured = std::env::var("NTFY_TOPIC")
        ...
        .unwrap_or(false)
        || {
            let env_file = home.join(".config/cream/env");
    ...
    Ok(ToolResult::json(&json!({
        ...
        "config_file":      home.join(".config/cream/env").display().to_string(),
        "note": match status {
            ...
            _                 => "Session offline — run cream-autostart or enable systemd service",
        },
    })))
}
...
pub fn session_restart(_args: &Value) -> Result<ToolResult, String> {
    let session_name = std::env::var("CREAM_SESSION")
    ...
    Ok(ToolResult::json(&json!({
        ...
        "note":         "tmux session will be killed in ~1 s. The cream watchdog will spawn a fresh Claude session and send a new email within ~30 s.",
