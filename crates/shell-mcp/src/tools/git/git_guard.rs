...
fn classify(tool: &str, args: &Value) -> Option<&'static str> {
    if matches!(
        ...
        // Determinism-layer mutations all flow through the same auto-checkpoint
        // class. dry_run mutations stay outside the checkpoint trail since they
        // don't actually touch disk.
...
fn write_audit(tool: &str, class: &str, status: &str, error: Option<&str>, repo_path: &Path) {
