...
pub fn pipeline_run(
    ...
    let step_defs: Vec<StepDef> = steps_raw
        .iter()
        .enumerate()
        .map(|(i, s)| {
            let id = s["id"].as_str().unwrap_or(&format!("step{i}")).to_string();
            let cmd = s["cmd"]
                .as_str()
    ...
    let step_list: Vec<Value> = pipeline
        .steps
        .iter()
        .map(|s| {
            json!({
...
pub fn pipeline_status(args: &Value, pipelines: &Arc<PipelineStore>) -> Result<ToolResult, String> {
    ...
    let steps: Vec<Value> = pipeline
        ...
        .iter()
        .map(|s| {
            let status = lock_mutex(&s.status, "pipeline step status").clone();
            let job_id = lock_mutex(&s.job_id, "pipeline step job id").clone();
            json!({
                "id":         s.id,
                "label":      s.label,
                "cmd":        s.cmd,
