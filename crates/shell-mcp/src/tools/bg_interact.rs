...
pub fn bg_send(args: &Value, store: &Arc<JobStore>) -> Result<ToolResult, String> {
    let job_id = args["job_id"].as_str().ok_or("bg_send: 'job_id' required")?;
    ...
    writer.flush()
        .map_err(|e| format!("bg_send: flush: {e}"))?;

    let sent = text.len() + if append_newline && !text.ends_with('\n') { 1 } else { 0 };
