...
pub fn cocotb_run(args: &Value) -> Result<ToolResult, String> {
    ...
    let (passed, failed) = if use_pytest {
        ...
        parse_results_xml(&dir_path.join("results.xml"))
...
fn parse_hw_targets(output: &str) -> Vec<Value> {
    ...
    for line in output.lines() {
        if let Some(t) = line.strip_prefix("TARGET: ") {
            ...
            let device = parts.first().copied().unwrap_or("").to_owned();
...
fn find_report(project_dir: &str, keyword: &str) -> Option<PathBuf> {
    ...
    for entry in iter.filter(|e| matches!(e.file_type, FileType::File)) {
        ...
        let mtime = std::fs::metadata(&entry.path)
            ...
            .ok();
        match (&best, mtime) {
            (None, Some(t)) => best = Some((entry.path, t)),
            (Some((_, prev_t)), Some(t)) if t > *prev_t => best = Some((entry.path, t)),
            (None, None) => best = Some((entry.path, std::time::UNIX_EPOCH)),
            _ => {}
        }
    }
    best.map(|(p, _)| p)
...
fn open_serial(port: &str, baud: u64, timeout_ms: u64) -> Result<std::fs::File, String> {
    let vtime = (timeout_ms / 100).clamp(1, 255).to_string();
