...
fn collect_checkpoints(dir: &Path, out: &mut Vec<PathBuf>, _depth: usize) {
    // Drive the unified walker, restricted to checkpoint extensions.
    // Gitignore handles target/, .git/, and dotfiles automatically.
    let opts = WalkOptions {
        root: dir.to_path_buf(),
        include: vec!["**/*.{pt,pth,ckpt,bin,safetensors}".to_owned()],
        max_depth: Some(6),
        ..WalkOptions::default()
    };
