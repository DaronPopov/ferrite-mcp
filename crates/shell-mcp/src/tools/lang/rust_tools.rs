...
pub fn test_run(args: &Value, state: &Arc<Mutex<ServerState>>) -> Result<ToolResult, String> {
    ...
    // -- separates cargo args from test binary args. Keep pretty output so
    // parse_test_output can read the stable "test name ... ok" lines.
    cmd.args(["--", "--format", "pretty", "--color", "never"]);
