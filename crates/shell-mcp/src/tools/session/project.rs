...
//!   chip_status          — scan all chips in a chip-workspace (a directory containing `chips/<name>/`)
...
//!
//! `chip_*`, `rtl_regression_*`, and `fpga_*` tools accept an optional `lab_path`
//! arg. If absent, the workspace is resolved by walking up from the server cwd
//! looking for a directory that contains a `chips/` subdirectory. If none is
//! found, the tools return a structured error rather than guessing — no personal
//! paths are baked in.
...
/// Resolve the chip workspace ("lab path") for the chip_*/rtl_*/fpga_* tools.
///
/// Resolution order:
///   1. explicit `lab_path` arg (tilde-expanded)
///   2. walk up from server cwd looking for a directory containing `chips/`
///   3. error — no personal paths are baked in
fn resolve_lab_path(args: &Value, state: &Arc<Mutex<ServerState>>) -> Result<PathBuf, String> {
    if let Some(p) = args["lab_path"].as_str() {
        return Ok(expand_tilde(p));
    }
    let cwd = resolve_or_cwd(state, None)?;
    let mut dir: &Path = &cwd;
    loop {
        if dir.join("chips").is_dir() {
    ...
    Err(format!(
        "no chip workspace found. Pass `lab_path` explicitly, or run from inside a directory \
         that contains a `chips/` subdirectory (searched upward from {}).",
        cwd.display()
    ))
}
...
fn detect_project_at(dir: &Path) -> Option<(String, String)> {
    ...
    // Structural detection only — no name-based heuristics, no personal paths.

    // Chip workspace: a directory containing `chips/<name>/`
    if dir.join("chips").is_dir() {
        return Some((name, "rtl_lab".to_owned()));
    }

    // RTL/FPGA project: contains rtl/, ip/, or top/ markers
    let rtl_markers = ["rtl", "ip", "top"];
    if rtl_markers.iter().any(|m| dir.join(m).is_dir()) {
        return Some((name, "rtl_project".to_owned()));
    }

    // Cargo workspace vs single crate
    if dir.join("Cargo.toml").exists() {
        // A `[workspace]` section indicates a cargo workspace; otherwise a single crate.
        ...
        return Some((name, kind.to_owned()));
    }

    // Python project
    if dir.join("pyproject.toml").exists() || dir.join("setup.py").exists() {
        return Some((name, "python_project".to_owned()));
    }

    // Node project
    if dir.join("package.json").exists() {
        return Some((name, "node_project".to_owned()));
    }
...
fn collect_active_targets(root: &Path, ptype: &str) -> Vec<Value> {
    ...
    } else if ptype == "rust_project" || ptype == "rust_workspace" {
...
fn context_hints_for(ptype: &str) -> Vec<&'static str> {
    match ptype {
        "rtl_lab" => vec![
            "Use chip_status to see all chips in this workspace",
            "Use chip_build_pipeline to run the RTL flow for one chip",
            "Use board_status to detect connected hardware",
        ],
        "rtl_project" => vec![
            "Use verilog_lint to check syntax",
            "Use vivado_tcl for synthesis",
            "Use fpga_program to flash a bitstream",
        ],
        "rust_workspace" => vec![
            "Use cargo_tree to inspect the workspace graph",
            "Use test_run for tests",
            "Use build_check for compile-only checks",
        ],
        "rust_project" => vec![
            "Use cargo_tree to inspect dependencies",
            "Use test_run for tests",
        ],
        "python_project" => vec!["Use test_run for pytest", "Use exec for ad-hoc scripts"],
        "node_project" => vec!["Use exec to run npm/yarn scripts"],
        _ => vec![],
    }
}
...
pub fn chip_status(args: &Value, state: &Arc<Mutex<ServerState>>) -> Result<ToolResult, String> {
    let lab_path = resolve_lab_path(args, state)?;
...
pub fn chip_build_pipeline(
    ...
    for step in &steps {
        ...
        if !was_success && !overall_success {
            break;
        }
    }
...
pub fn fpga_triage(args: &Value, state: &Arc<Mutex<ServerState>>) -> Result<ToolResult, String> {
    let chip = args["chip"]
        .as_str()
...
pub fn fpga_artifacts(args: &Value, state: &Arc<Mutex<ServerState>>) -> Result<ToolResult, String> {
    let chip = args["chip"]
        .as_str()
...
fn discover_bitstream(chip: &str, chip_path: &Path) -> Option<PathBuf> {
    // Drive the unified walker over build/, restricted to *.bit files.
    // Prefers a bitstream whose name contains the chip identifier and
    // falls back to the first *.bit found.
    let opts = WalkOptions {
        root: chip_path.join("build"),
        include: vec!["**/*.bit".to_owned()],
        ..WalkOptions::default()
    };
    let iter = walk::walk_sequential(&opts).ok()?;
