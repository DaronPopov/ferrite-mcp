...
fn discover_projects(root: &str) -> Vec<Value> {
    ...
    for entry in entries.filter_map(|e| e.ok()) {
        ...
        if let Some(proj) = classify_project(&path, &name) {
            ...
            // Might be a workspace container (a directory of repos) — scan one level deeper
...
/// chips: scan a chip workspace if one is found by walking up from cwd looking
/// for a directory containing `chips/`. Returns `null` if none is found — no
/// personal paths are baked in.
fn orient_chips(
    cwd: &str,
    state: &std::sync::Arc<std::sync::Mutex<crate::server::ServerState>>,
) -> Value {
    let Some(lab_path) = find_chip_workspace(cwd) else {
        return json!(null);
    };
    match project::chip_status(&json!({ "lab_path": lab_path }), state) {
        Ok(tr) => parse_tool_result_json(&tr),
        Err(e) => json!({ "error": e }),
    }
}

/// Walk up from `cwd` looking for a directory that contains a `chips/`
/// subdirectory. This is the structural marker of a chip workspace.
fn find_chip_workspace(cwd: &str) -> Option<String> {
    let mut p = std::path::PathBuf::from(cwd);
    loop {
        if p.join("chips").is_dir() {
