...
fn detect_build_cmd(project: &str) -> String {
    // Structural detection only — check for marker files in the project directory.
    // No project-name heuristics. The remote `~` is expanded by the remote shell.
    let p = format!("~/{project}");
    if std::path::Path::new(&format!("{p}/Cargo.toml")).exists() {
        "cargo build --release 2>&1".to_owned()
    } else if std::path::Path::new(&format!("{p}/package.json")).exists() {
        "npm run build 2>&1".to_owned()
    } else if std::path::Path::new(&format!("{p}/Makefile")).exists() {
        "make 2>&1".to_owned()
    } else if std::path::Path::new(&format!("{p}/chips")).exists() {
        // Chip workspace — caller should use chip_build_pipeline instead
        "echo 'Use chip_build_pipeline for chip workspaces' 2>&1".to_owned()
    } else {
        // No marker — let the caller specify a `cmd` arg explicitly
        "echo 'No build command detected; pass `cmd` arg explicitly' 2>&1".to_owned()
    }
}
